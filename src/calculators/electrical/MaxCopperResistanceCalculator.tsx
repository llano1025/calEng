  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="bg-gray-50 p-4 rounded mb-6">
        <h3 className="font-medium mb-3">Circuit Parameters:</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1">Line Voltage (V):</label>
            <input
              type="number"
              value={lineVoltage || ''}
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Power Factor (cosθ):</label>
            <input
              type="number"
              value={powerFactor || ''}
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Max. Loss Percentage (%):</label>
            <input
              type="number"
      ...
      {/* Harmonic Parameters */}
      <div className="bg-gray-50 p-4 rounded mb-6">
        <h3 className="font-medium mb-3">Harmonic Parameters:</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1">Total Harmonic Distortion (%):</label>
            <input
              type="number"
              value={thd || ''}
          ...
          <div className="flex items-center">
            <input
              ...
              className="mr-2"
            />
            <label htmlFor="includeNeutral" className="text-sm font-medium">
              Include Neutral Conductor
            </label>
          </div>
          {includeNeutral && (
            <div>
              <label className="block text-sm font-medium mb-1">Neutral/Phase Current Ratio:</label>
              <input
                type="number"
      ...
      {maxResistance !== null && (
        <div className="bg-blue-50 p-4 rounded border border-blue-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {includeNeutral && (
              <div>
                ...
                <p className="text-lg">{(fundamentalCurrent !== null ? fundamentalCurrent * neutralCurrentRatio : 0).toFixed(2)} A</p>
              </div>
            )}
            <div>
              <p className="text-sm font-medium">Maximum Resistance:</p>
              <p className="text-lg">{maxResistance.toFixed(4)} mΩ/m</p>
            </div>
            {copperLoss !== null && (
              <div>
          ...
          <div className="mt-6 border-t border-blue-200 pt-4">
            <h3 className="font-medium mb-2">Detailed Calculation:</h3>
            <div className="bg-white p-3 rounded border border-gray-200 text-sm">
              ...
              <ul className="list-disc pl-5 mb-3 space-y-1">
                ...
                {includeNeutral && <li>Neutral current (IN): {(fundamentalCurrent !== null ? fundamentalCurrent * neutralCurrentRatio : 0).toFixed(2)} A</li>}
              ...
              <ol className="list-decimal pl-5 space-y-1">
                ...
                <li>
                  {includeNeutral ? (
                    <>
                      ...
                      = (3 × {designCurrent}² + {(fundamentalCurrent !== null ? fundamentalCurrent * neutralCurrentRatio : 0).toFixed(2)}²) × {cableLength}<br />
                      = {(3 * Math.pow(designCurrent, 2) + Math.pow((fundamentalCurrent !== null ? fundamentalCurrent * neutralCurrentRatio : 0), 2)).toFixed(2)} × {cableLength}<br />
                      = {(3 * Math.pow(designCurrent, 2) + Math.pow((fundamentalCurrent !== null ? fundamentalCurrent * neutralCurrentRatio : 0), 2)) * cableLength}
                    </>
                  ) : (
                    <>
                ...
                <li>
                  ...
                  = {maxLossPercentage/100} × √3 × {lineVoltage} × {fundamentalCurrent !== null ? fundamentalCurrent.toFixed(2) : 0} × {powerFactor} × 1000<br />
                  = {maxLossPercentage/100} × {Math.sqrt(3).toFixed(4)} × {lineVoltage} × {fundamentalCurrent !== null ? fundamentalCurrent.toFixed(2) : 0} × {powerFactor} × 1000<br />
                  = {(maxLossPercentage/100 * Math.sqrt(3) * lineVoltage * (fundamentalCurrent !== null ? fundamentalCurrent : 0) * powerFactor * 1000).toFixed(2)}
                </li>
                <li>
                  Calculate max resistance: numerator ÷ denominator<br />
                  = {(maxLossPercentage/100 * Math.sqrt(3) * lineVoltage * (fundamentalCurrent !== null ? fundamentalCurrent : 0) * powerFactor * 1000).toFixed(2)} ÷ {includeNeutral ? 
                    ((3 * Math.pow(designCurrent, 2) + Math.pow((fundamentalCurrent !== null ? fundamentalCurrent * neutralCurrentRatio : 0), 2)) * cableLength).toFixed(2) : 
                    (3 * Math.pow(designCurrent, 2) * cableLength).toFixed(2)}<br />
                  = {maxResistance !== null ? maxResistance.toFixed(4) : 0} mΩ/m
                </li>
              </ol>
            </div>
          </div>
          ...
          <div className="mt-4">
            <p className="text-sm text-gray-600">
              Select appropriate conductor size from TG Table 7.8 based on the calculated resistance value.
            </p>
          </div>
        </div>
      )}
